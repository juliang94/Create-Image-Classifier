...
parser.add_argument('--topk', help = 'input how many classes probabilities to display', type = int)
...
print('You may use the ', device, 'to calculate predictions')
...
img = process_image(test_dir + args.image_dir)
imshow(img)
...
def sanity_check(img_path, model):
    ...
    probs, classes, labels = predict(img_path, model, topk = args.topk)
